...
import { GENERATION_QUEUE_NAME, REFINEMENT_QUEUE_NAME, MEDIA_QUEUE_NAME, } from './queues';
...
async function startWorkers() {
    ...
    const generationWorker = new Worker(GENERATION_QUEUE_NAME, processGenerationJob, {
        connection: redisConnection,
        concurrency: 5,
    });
    const refinementWorker = new Worker(REFINEMENT_QUEUE_NAME, processRefinementJob, {
        connection: redisConnection,
        concurrency: 3,
    });
    const mediaWorker = new Worker(MEDIA_QUEUE_NAME, processMediaJob, {
        connection: redisConnection,
        concurrency: 2,
    });
