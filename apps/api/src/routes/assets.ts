...
const ASSET_TYPES = ['IMAGE', 'VIDEO', 'AUDIO', 'REFERENCE'] as const
    ...
    const assets = await prisma.asset.findMany({
      where: {
        AND: [
          ...
          upperType && ASSET_TYPES.includes(upperType as (typeof ASSET_TYPES)[number])
            ? { type: upperType as (typeof ASSET_TYPES)[number] }
            : {},
        ],
      },
